...
namespace LealPassword.Database.Logic
{
    public sealed class RegisterManagement : Interfaces.IRegisterManagement
    {
        ...
        public void DeleteRegister(Register register)
            => _resource.DeleteRegister(register.Encrypt(_unhashedPassword));

        public void InsertRegister(Register register)
            => _resource.InsertRegister(register.Encrypt(_unhashedPassword));

        public void UpdateRegister(Register register)
            => _resource.UpdateRegister(register.Encrypt(_unhashedPassword));

        public List<Register> GetRegisters()
            => _resource.GetRegisters().Select(enReg => enReg.Decrypt(_unhashedPassword)).ToList();
        #region Dispose
